...
- **Packaging Windows** (`pyinstaller gestion_app.spec`, `installer.iss`) est
  ...
  est **un seul fichier** `dist/GestionCommerciale.exe` (onefile). Artefact CI :
  `GestionCommerciale-Windows`. Si `ModuleNotFoundError: main_window` au
  lancement : exclusion antivirus sur l'exe.
