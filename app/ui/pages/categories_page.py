...
from PySide6.QtWidgets import QAbstractItemView, QHBoxLayout, QHeaderView, QInputDialog, QLineEdit, QPushButton, QTableWidget, QTableWidgetItem, QVBoxLayout, QWidget
...
class CategoriesPage(QWidget):
    ...
    def _build_categories(self) -> QWidget:
        ...
        self.cat_table.setHorizontalHeaderLabels([t('categories.name'), t('categories.description')])
    ...
    def _edit_category(self) -> None:
        ...
        if not cat_id:
            warn(self, t('Sélectionnez une catégorie.'))
            return
        category = CategoryController.get(cat_id)
        dialog = CategoryDialog(category=category, parent=self)
        if dialog.exec() and dialog.data:
            CategoryController.update(cat_id, dialog.data['name'], dialog.data['description'])
            self.refresh()
            self.state.notify_data_changed()
    ...
    def _delete_category(self) -> None:
        ...
        if not self.state.can(perms.MANAGE_CATEGORIES):
            warn(self, t("Vous n'avez pas l'autorisation de supprimer une catégorie."))
            return
        if confirm(self, t('Supprimer cette catégorie ? Les produits seront conservés.')):
    ...
    def _build_units(self) -> QWidget:
        ...
        self.unit_table.setHorizontalHeaderLabels([t('categories.units'), t('Type')])
        self.unit_table.horizontalHeader().setSectionResizeMode(0, QHeaderView.ResizeMode.Stretch)
    ...
    def _delete_unit(self) -> None:
        ...
        if row < 0 or row >= len(self._unit_ids):
            warn(self, t('Sélectionnez une unité.'))
            return
        if not self.state.can(perms.MANAGE_CATEGORIES):
            warn(self, t("Vous n'avez pas l'autorisation de supprimer une unité."))
            return
    ...
    def refresh(self) -> None:
        ...
        for row, unit in enumerate(units):
            ...
            self.unit_table.setItem(row, 1, QTableWidgetItem('Par défaut' if unit.is_default else 'Personnalisée'))
