...
from PySide6.QtWidgets import QAbstractItemView, QComboBox, QDoubleSpinBox, QFormLayout, QHBoxLayout, QHeaderView, QLineEdit, QPushButton, QTableWidget, QTableWidgetItem, QVBoxLayout, QWidget
...
class PurchasesPage(QWidget):
    ...
    def __init__(self, state: AppState):
        ...
        layout.addWidget(page_title(t('Achats / Réceptions')))
        form_w = QWidget()
        ...
        self.qty.setRange(0.001, 10000000)
        self.qty.setDecimals(3)
        self.qty.setValue(1)
        self.cost = QDoubleSpinBox()
        self.cost.setRange(0, 1000000000)
        self.paid = QDoubleSpinBox()
        self.paid.setRange(0, 1000000000)
        self.note = QLineEdit()
        form.addRow(t('Fournisseur'), self.supplier)
        form.addRow(t('N° facture'), self.invoice)
        form.addRow(t('Produit'), self.product)
        form.addRow(t('Quantité'), self.qty)
        form.addRow(t('Coût unitaire'), self.cost)
        form.addRow(t('Montant payé'), self.paid)
        form.addRow(t('Note'), self.note)
        layout.addWidget(make_card(form_w))
        btns = QHBoxLayout()
        add_line = QPushButton(t('Ajouter la ligne'))
        add_line.clicked.connect(self._add_line)
        save = QPushButton(t("Enregistrer l'achat"))
        ...
        self.lines_table.setHorizontalHeaderLabels([t('Produit'), t('Qté'), t('Coût U.'), t('Total')])
        ...
        self.history.setHorizontalHeaderLabels([t('Date'), t('Fournisseur'), t('Facture'), t('Total'), t('Payé'), t('Statut')])
        ...
        self.cancel_purchase_button = QPushButton(t("Annuler l'achat sélectionné"))
    ...
    def refresh(self) -> None:
        ...
        self.supplier.clear()
        self.supplier.addItem(t('— Aucun —'), None)
    ...
    def _save(self) -> None:
        if not self.state.can(perms.MANAGE_PURCHASES):
            warn(self, t('Autorisation insuffisante.'))
            return
        if not self._lines:
            warn(self, t('Ajoutez au moins une ligne.'))
            return
        try:
            purchase = PurchaseService.create(list(self._lines), supplier_id=self.supplier.currentData(), invoice_number=self.invoice.text(), amount_paid=self.paid.value(), note=self.note.text(), user_id=self.state.user_id, username=getattr(self.state.current_user, 'username', ''))
    ...
    def _cancel_selected_purchase(self) -> None:
        ...
        if row < 0 or row >= len(self._purchase_ids):
            warn(self, t("Sélectionnez un achat dans l'historique."))
            return
        if not self.state.can(perms.MANAGE_PURCHASES):
            warn(self, t('Autorisation insuffisante.'))
            return
        purchase_id = self._purchase_ids[row]
        if not confirm(self, f"Annuler l'achat #{purchase_id} ?\n\nLe stock reçu sera retiré et la dette fournisseur liée sera annulée si aucun remboursement n'existe."):
            return
        try:
            PurchaseService.cancel_purchase(purchase_id, user_id=self.state.user_id, username=getattr(self.state.current_user, 'username', ''))
        except ValueError as exc:
            warn(self, str(exc))
            return
        self._reload_history()
