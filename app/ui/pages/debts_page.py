...
from PySide6.QtWidgets import QAbstractItemView, QComboBox, QHBoxLayout, QHeaderView, QLabel, QLineEdit, QPushButton, QTableWidget, QTableWidgetItem, QTabWidget, QVBoxLayout, QWidget
...
STATUS_LABELS = {STATUS_OPEN: 'En cours', STATUS_PARTIAL: 'Partiellement payée', STATUS_PAID: 'Soldée', STATUS_CANCELLED: 'Annulée'}
DEBT_TABS = (('Non payées', 'unpaid'), ('Payées', 'paid'), ('Échues', 'overdue'), ('Tout', 'all'))
SORT_CHOICES = (('Plus récentes', 'recent'), ('Échéance', 'due_date'), ('Montant restant', 'remaining'), ('Client A → Z', 'client'))

class DebtsPage(QWidget):
    """Registre des dettes avec filtres par onglet et tri persistant."""
    HEADERS = ['Date', 'Client', 'Téléphone', 'Initial', 'Reste', 'Échéance', 'Statut', 'Vente', 'Note']
    ...
    def __init__(self, state: AppState):
        ...
        root.setSpacing(12)
        header = QHBoxLayout()
        header.addWidget(page_title(t('Dettes clients')))
        header.addStretch()
        self.add_debt_btn = QPushButton(t('+ Nouvelle dette'))
        self.add_debt_btn.setObjectName('Primary')
        self.add_debt_btn.setToolTip(t("Saisie libre hors caisse — réservée à l'administrateur"))
        ...
        self.search.setPlaceholderText(t('Rechercher (client, téléphone, note)…'))
        ...
        tools.addWidget(QLabel(t('Trier :')))
        ...
        actions = QHBoxLayout()
        actions.addStretch()
        self.settle_btn = QPushButton(t('Payé'))
        self.settle_btn.setObjectName('Success')
        self.settle_btn.setToolTip(t('Enregistrer un règlement (tous les rôles)'))
        self.settle_btn.clicked.connect(self._settle)
        self.history_btn = QPushButton(t('Historique client'))
    ...
    def refresh(self) -> None:
        ...
        debts = DebtService.list_debts(search=self.search.text().strip(), filter_mode=mode, sort_by=sort_by, limit=2000)
        ...
        for row, debt in enumerate(debts):
            ...
            values = [format_datetime(debt.created_at) if debt.created_at else '—', client_name, phone, format_money(debt.amount_initial, currency), format_money(remaining, currency), due, status, ticket, debt.note or '—']
        ...
        labels = {'unpaid': 'non payées', 'paid': 'payées / soldées', 'overdue': 'échues', 'all': 'au total'}
        self.summary.setText(f'{len(debts)} dette(s) {labels.get(mode, '')} — reste dû affiché (actives) : <b>{format_money(total_remaining, currency)}</b> — tri : {self.sort_combo.currentText()}')
    ...
    def _settle(self) -> None:
        ...
        if not debt_id:
            warn(self, t('Sélectionnez une dette.'))
            return
        debt = DebtService.get(debt_id)
        if not debt or not debt.is_active or float(debt.amount_remaining or 0) <= 0:
            warn(self, t("Cette dette n'a plus de reste dû."))
            return
        client = debt.client or ClientController.get(debt.client_id)
        name = client.name if client else f'Client #{debt.client_id}'
        dialog = DebtPaymentDialog(name, float(debt.amount_remaining), parent=self)
        if not dialog.exec() or not dialog.result_data:
            return
        try:
            payment = DebtService.pay_debt(debt.id, dialog.result_data['amount'], payment_method=dialog.result_data['payment_method'], note=dialog.result_data['note'], user_id=self.state.user_id, username=getattr(self.state.current_user, 'username', ''))
        except ValueError as exc:
            warn(self, str(exc))
            return
        ...
        DebtPaymentReceiptDialog(client_name=name, amount=dialog.result_data['amount'], payment_method=dialog.result_data['payment_method'], remaining_after=remaining, note=dialog.result_data['note'], cashier=getattr(self.state.current_user, 'username', '') or '', payment_id=getattr(payment, 'id', None), parent=self).exec()
        self.refresh()
        self.state.notify_data_changed()
