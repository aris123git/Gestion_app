...
class AuthorizeDialog(QDialog):
    ...
    def __init__(self, reason: str='', parent=None):
        ...
        self.setWindowTitle(t('Autorisation requise'))
        self.setModal(True)
        ...
        intro = QLabel(reason or "Cette action nécessite l'autorisation d'un administrateur.")
        intro.setWordWrap(True)
        layout.addWidget(intro)
        form = QFormLayout()
        form.setSpacing(10)
        self.username = QLineEdit()
        self.username.setPlaceholderText(t("Nom d'utilisateur administrateur"))
        self.password = QLineEdit()
        self.password.setEchoMode(QLineEdit.EchoMode.Password)
        self.password.setPlaceholderText(t('Mot de passe'))
        form.addRow(t('Administrateur'), self.username)
        form.addRow(t('Mot de passe'), self.password)
        layout.addLayout(form)
        buttons = QHBoxLayout()
        cancel = QPushButton(t('Annuler'))
        cancel.clicked.connect(self.reject)
        confirm_btn = QPushButton(t('Autoriser'))
    ...
    def _authorize(self) -> None:
        ...
        if not username or not password:
            warn(self, t("Saisissez le nom d'utilisateur et le mot de passe."))
            return
        if not AuthService.verify_admin_password(username, password):
            warn(self, t('Identifiants administrateur invalides.'))
            self.password.clear()
