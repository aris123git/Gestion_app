"""Saisie commande table Maquis — même catalogue que la caisse / tablette."""
...
class TableOrderDialog(QDialog):
    def __init__(self, order_id: int, state: AppState, parent=None):
        super().__init__(parent)
        self.state = state
        self.order_id = order_id
        self.setWindowTitle(t("Commande table"))
        ...
        close_btn = QPushButton(t("Fermer"))
    ...
    def _reload_lines(self) -> None:
        ...
        self.header.setText(f"{order.public_id} · {table_name}")
    ...
    def _add_free_amount(self, product) -> None:
        ...
        if sale_price <= 0:
            warn(
                self,
                f"« {product.name} » : définissez un prix de référence pour le montant libre.",
            )
            return
