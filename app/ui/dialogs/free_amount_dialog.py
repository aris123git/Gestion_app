...
from PySide6.QtWidgets import QDialog, QDoubleSpinBox, QGridLayout, QHBoxLayout, QLabel, QPushButton, QVBoxLayout
...
class FreeAmountDialog(QDialog):
    ...
    def __init__(self, product_name: str, sale_price: float, parent=None):
        ...
        if self.sale_price > 0:
            hint = QLabel(f'Prix de référence : {format_money(self.sale_price, self.currency)} / unité\nSaisissez le montant demandé par le client.')
        else:
        ...
        self.custom.setRange(0, 1000000000)
    ...
    def _update_estimate(self) -> None:
        ...
        self.estimate.setText(f'Quantité estimée : {qty:g} unité(s) de vente ({format_money(amount, self.currency)} ÷ {format_money(self.sale_price, self.currency)})')
