...
from app.utils.cancel_reason import MIN_CANCEL_REASON_LETTERS, count_letters, validate_cancel_reason
...
class CancelSaleDialog(QDialog):
    ...
    def __init__(self, ticket_number: str, parent=None):
        ...
        self.setWindowTitle(t('Annuler la vente'))
        self.setModal(True)
        ...
        info = QLabel(f"<b>Annuler la vente {ticket_number} ?</b><br/><br/>Les articles seront remis en stock et la vente ne comptera plus dans le chiffre d'affaires.<br/><br/>Un <b>motif obligatoire</b> (au moins 10 lettres) est requis.")
        info.setWordWrap(True)
        layout.addWidget(info)
        layout.addWidget(QLabel(t("Motif de l'annulation")))
        self.motif = QPlainTextEdit()
        self.motif.setPlaceholderText(t("Ex. : erreur de saisie, client a changé d'avis, doublon…"))
        ...
        buttons = QHBoxLayout()
        cancel = QPushButton(t('Retour'))
        cancel.clicked.connect(self.reject)
        self.confirm = QPushButton(t("Valider l'annulation"))
    ...
    def _update_counter(self) -> None:
        ...
        self.counter.setText(f'{n} / {MIN_CANCEL_REASON_LETTERS} lettres minimum' + (' — OK' if ok else ''))
        self.counter.setStyleSheet('color: #16a34a;' if ok else 'color: #b45309;')

    def _save(self) -> None:
        reason = validate_cancel_reason(self.motif.toPlainText())
        if not reason:
            warn(self, t("Indiquez un motif d'au moins 10 lettres (pas seulement des chiffres ou des signes)."))
            return
        self.reason = reason
        self.accept()
