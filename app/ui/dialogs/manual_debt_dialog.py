...
from PySide6.QtWidgets import QCheckBox, QDateEdit, QDialog, QDoubleSpinBox, QFormLayout, QHBoxLayout, QLabel, QLineEdit, QMessageBox, QPushButton, QVBoxLayout
...
class ManualDebtDialog(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        self.setWindowTitle(t('Nouvelle dette (hors caisse)'))
        self.setModal(True)
        ...
        hint = QLabel(t("Saisie libre réservée à l'administrateur — sans passer par la caisse."))
        ...
        form.addRow(t('Client'), self.client_search)
        ...
        self.note = QLineEdit()
        self.note.setPlaceholderText(t('Motif / commentaire (facultatif)'))
        form.addRow(t('Note'), self.note)
        layout.addLayout(form)
        buttons = QHBoxLayout()
        cancel = QPushButton(t('Annuler'))
        cancel.clicked.connect(self.reject)
        save = QPushButton(t('Enregistrer la dette'))
    ...
    def _resolve_client_id(self) -> Optional[int]:
        ...
        if phone:
            ...
            if existing:
                ...
                return existing.id
        answer = QMessageBox.question(self, 'Nouveau client', f'Aucun client trouvé pour « {typed} ».\n\nCréer une fiche client pour cette dette ?', QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No, QMessageBox.StandardButton.Yes)
    ...
    def _save(self) -> None:
        ...
        if amount <= 0:
            warn(self, t('Indiquez un montant supérieur à zéro.'))
            return
        client_id = self._resolve_client_id()
        if not client_id:
            warn(self, t('Sélectionnez ou créez un client.'))
            return
        due = None
        if self.due_enabled.isChecked():
            due = self.due_date.date().toPython()
        note = self.note.text().strip() or 'Dette manuelle (hors caisse)'
        self.result_data = {'client_id': client_id, 'amount': amount, 'due_date': due, 'note': note}
        self.accept()
