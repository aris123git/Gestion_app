...
from PySide6.QtWidgets import QAbstractItemView, QComboBox, QDialog, QHBoxLayout, QHeaderView, QLabel, QPushButton, QTableWidget, QTableWidgetItem, QTabWidget, QVBoxLayout, QWidget
...
SORT_CHOICES = (('Plus récentes', 'recent'), ('Échéance', 'due_date'), ('Montant restant', 'remaining'))
...
class DebtHistoryDialog(QDialog):
    ...
    def __init__(self, client_id: int, client_name: str, parent=None):
        ...
        self.summary_label = QLabel(f'Solde : <b>{format_money(summary['total_remaining'], currency)}</b> — {summary['active_count']} dette(s) active(s) — {summary['overdue_count']} échéance(s) dépassée(s)')
        ...
        self.debts_table.setHorizontalHeaderLabels([t('Date'), t('Montant initial'), t('Reste'), t('Échéance'), t('Statut'), t('Vente'), t('Note')])
        self.debts_table.horizontalHeader().setSectionResizeMode(6, QHeaderView.ResizeMode.Stretch)
        self.debts_table.setEditTriggers(QAbstractItemView.EditTrigger.NoEditTriggers)
        self.debts_table.setSelectionBehavior(QAbstractItemView.SelectionBehavior.SelectRows)
        ...
        main_tabs.addTab(debts_wrap, t('Dettes'))
        main_tabs.addTab(self._build_payments_tab(), t('Remboursements'))
    ...
    def _build_payments_tab(self) -> QWidget:
        ...
        self.payments_table.setHorizontalHeaderLabels([t('Date'), t('Montant'), t('Mode'), t('Dette #'), t('Note')])
    ...
    def _reload_debts(self) -> None:
        ...
        debts = DebtService.list_debts(client_id=self.client_id, filter_mode=self._filter_mode(), sort_by=sort_by, limit=1000)
        ...
        for row, debt in enumerate(debts):
            ...
            if overdue:
                status += ' (échue)'
            values = [format_datetime(debt.created_at) if debt.created_at else '—', format_money(debt.amount_initial, currency), format_money(debt.amount_remaining, currency), due, status, ticket or '—', debt.note or '—']
    ...
    def _load_payments(self) -> None:
        ...
        for row, payment in enumerate(payments):
            values = [format_datetime(payment.payment_date), format_money(payment.amount, currency), payment.payment_method or '—', str(payment.debt_id), payment.note or '—']
            for col, value in enumerate(values):
