...
from PySide6.QtWidgets import QDialog, QDoubleSpinBox, QFormLayout, QHBoxLayout, QLabel, QLineEdit, QPushButton, QVBoxLayout
...
class OpenCashSessionDialog(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        self.setWindowTitle(t('Ouverture de caisse'))
        self.setModal(True)
        ...
        layout.addWidget(QLabel(t('Indiquez le <b>fond de caisse</b> (espèces déjà dans le tiroir) avant de commencer.')))
        form = QFormLayout()
        self.amount = QDoubleSpinBox()
        self.amount.setRange(0, 1000000000)
        self.amount.setDecimals(0)
        self.amount.setSuffix(f' {currency}')
        form.addRow(t('Fond de caisse'), self.amount)
        layout.addLayout(form)
        buttons = QHBoxLayout()
        ok = QPushButton(t('Ouvrir la caisse'))
...
class CloseCashSessionDialog(QDialog):
    ...
    def __init__(self, session_id: int, expected: float, parent=None):
        super().__init__(parent)
        self.setWindowTitle(t('Fermeture de caisse'))
        self.setModal(True)
        ...
        self.note = QLineEdit()
        self.note.setPlaceholderText(t('Commentaire (obligatoire si écart important)'))
        form.addRow(t('Montant compté'), self.counted)
        ...
        ok = QPushButton(t('Fermer la caisse'))
    ...
    def _update_variance(self) -> None:
        ...
        self.variance_label.setText(f"<span style='color:{color};'>Écart : {format_money(variance, currency)}</span>")
    ...
    def _save(self) -> None:
        ...
        if variance >= threshold + 0.009 and len(note) < 5:
            warn(self, f'Écart ≥ {threshold:g} : une note explicative est obligatoire (au moins 5 caractères).')
            self.note.setFocus()
            return
        self.result_data = {'counted': counted, 'note': note}
        self.accept()
...
def ensure_cash_session_open(parent, state) -> bool:
    ...
    if not dialog.exec() or dialog.opening_float is None:
        warn(parent, t('La caisse doit être ouverte pour continuer.'))
        return False
    try:
        CashSessionService.open_session(user.id, dialog.opening_float, username=getattr(user, 'username', ''))
    except ValueError as exc:
        warn(parent, str(exc))
        return False
    info(parent, t('Caisse ouverte.'))
    return True
...
def close_cash_session_flow(parent, state) -> bool:
    ...
    if not dialog.exec() or not dialog.result_data:
        return False
    try:
        closed = CashSessionService.close_session(open_sess.id, dialog.result_data['counted'], note=dialog.result_data['note'], user_id=user.id, username=getattr(user, 'username', ''))
    except ValueError as exc:
        warn(parent, str(exc))
        return False
    currency = settings_service.get_currency()
    info(parent, f'Caisse fermée.\nAttendu : {format_money(closed.expected_cash, currency)}\nCompté : {format_money(closed.closing_counted, currency)}\nÉcart : {format_money(closed.variance, currency)}')
    return True
